...
$rulename="DisasterRecovery1"



$N1="K3" #Change the Name according to need
$N2="K6" #Change the Name according to need
...
$nsg2 | Add-AzNetworkSecurityRuleConfig -Name $rulename -Description "AllowCommFromD1toD2" -Access Allow `
...
$nsg1 | Add-AzNetworkSecurityRuleConfig -Name $rulename -Description "AllowCommFromD1toD2" -Access Allow `
    -Protocol * -Direction Outbound -Priority 3891 -SourceAddressPrefix "10.0.2.0/24" -SourcePortRange $port1 `
    -DestinationAddressPrefix "15.0.2.0/24" -DestinationPortRange $port1

# Add the inbound security rule.
$nsg1 | Add-AzNetworkSecurityRuleConfig -Name $rulename -Description "AllowCommFromD2toD1" -Access Allow `
...
# Add the outbound security rule.
$nsg2 | Add-AzNetworkSecurityRuleConfig -Name $rulename -Description "AllowCommFromD2toD1" -Access Allow `
